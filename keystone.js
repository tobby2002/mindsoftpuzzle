...
keystone.init({
	...
	'auto update': true,
	'mongo': process.env.MONGO_URI || 'mongodb://neo:neo123@localhost/' + pkg.name,
...
keystone.set('email locals', {
	...
	host: (function() {
		...
		if (keystone.get('env') === 'production') return 'http://mindmap.ai';
		return (keystone.get('host') || 'http://localhost:') + (keystone.get('port') || '80');
		// return (keystone.get('host') || 'http://45.76.103.214:') + (keystone.get('port') || '80');
	})()
});
